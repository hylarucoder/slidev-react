...
type EnvelopeInput = Omit<PresentationEnvelope, 'version' | 'sessionId' | 'senderId' | 'seq' | 'timestamp'>
...
export function usePresentationSync({
  ...
  useEffect(() => {
    if (!session.enabled || !session.sessionId || (session.role !== 'presenter' && session.role !== 'viewer'))
      return

    if (!canSend(session.syncMode) && !canReceive(session.syncMode))
      return
    ...
    const removePeer = (senderId: string) => {
      if (!peerLastSeenRef.current.delete(senderId))
        return
    ...
    const sendSnapshot = () => {
      if (!canSend(session.syncMode) || !canAuthorState(session.role))
        return
    ...
    const onIncomingEnvelope = (incoming: unknown) => {
      ...
      if (envelope.type === 'session/join' && canSend(session.syncMode) && canAuthorState(session.role)) {
        sendSnapshot()
        return
      }
      ...
      if (envelope.type === 'state/snapshot' || envelope.type === 'state/patch') {
        ...
        if (updateAt <= lastRemoteUpdateRef.current)
          return
    ...
    const connectWebSocket = () => {
      ...
      const onOpen = () => {
        ...
        if (canAuthorState(session.role))
          sendSnapshot()
      }
    ...
    peerSweepIntervalId = window.setInterval(() => {
      ...
      for (const [peerId, lastSeenAt] of peerLastSeenRef.current) {
        if ((now - lastSeenAt) > PEER_STALE_AFTER_MS)
  ...
  }, [session.enabled, session.role, session.senderId, session.sessionId, session.syncMode, session.wsUrl])
  ...
  useEffect(() => {
    ...
    if (!hasPageChange && !hasClicksChange && !hasClicksTotalChange && !hasTimerChange && !hasDrawingsChange && !hasCursorChange)
      return

    if (hasPageChange || hasClicksChange || hasClicksTotalChange || hasTimerChange || hasDrawingsChange) {
      sendEnvelope({
        type: 'state/patch',
        payload: {
          state: {
