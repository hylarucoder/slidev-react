...
function isDrawStroke(value: unknown): value is PresentationDrawStroke {
  ...
  if (typeof value.id !== 'string' || typeof value.color !== 'string' || typeof value.width !== 'number')
    return false

  if ('kind' in value && value.kind !== 'pen' && value.kind !== 'circle' && value.kind !== 'rectangle')
    return false
...
function isStatePatchLike(value: unknown): value is Partial<PresentationSharedState> {
  ...
  if ('page' in value && typeof value.page !== 'number')
    return false

  if ('clicks' in value && typeof value.clicks !== 'number')
    return false

  if ('clicksTotal' in value && typeof value.clicksTotal !== 'number')
    return false

  if ('timer' in value && typeof value.timer !== 'number')
    return false

  if ('drawingsRevision' in value && typeof value.drawingsRevision !== 'number')
    return false

  if ('drawings' in value && !isDrawingsState(value.drawings))
    return false

  if ('lastUpdate' in value && typeof value.lastUpdate !== 'number')
    return false
  ...
  if ('cursor' in value) {
    ...
    if (typeof value.cursor.x !== 'number' || typeof value.cursor.y !== 'number')
      return false
  }
...
export function parsePresentationEnvelope(value: unknown): PresentationEnvelope | null {
  ...
  if (type === 'state/snapshot') {
    if (!isStatePatchLike(payload.state))
      return null
