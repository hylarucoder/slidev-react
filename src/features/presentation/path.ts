...
const ROLE_SET: ReadonlySet<PresentationPathRole> = new Set([
  'presenter',
])
...
export function parseStandalonePath(pathname: string): ParsedStandalonePath | null {
  ...
  if (Number.isNaN(maybeSlideNumber) || maybeSlideNumber < 1)
    return null
...
export function resolvePresentationBasePath(pathname: string) {
  ...
  if (parsed)
    return normalizeBasePath(parsed.basePath)

  const parsedStandalone = parseStandalonePath(pathname)
  if (parsedStandalone)
    return normalizeBasePath(parsedStandalone.basePath)
...
export function buildRolePathFromBase(
  ...
  const safeSlideNumber = Number.isFinite(slideNumber) && slideNumber > 0
    ? Math.floor(slideNumber)
    : 1

  if (!normalizedBase)
    return `/${role}/${safeSlideNumber}`
