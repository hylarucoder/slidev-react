...
export function MinimaxReactVisualizer() {
  ...
  const camera = useMemo(() => {
    if (step <= 0)
      return { x: 0, y: 0, perspective: 900 }
    if (step === 1)
      return { x: 10, y: 25, perspective: 860 }
    if (step === 2)
      return { x: 14, y: 32, perspective: 840 }
    if (step === 3)
      return { x: 20, y: 39, perspective: 820 }
  ...
  return (
    <section className="relative -m-14 flex h-[calc(100%+7rem)] w-[calc(100%+9rem)] overflow-hidden bg-[#1c1c1c] text-white">
      <div className="flex min-w-0 flex-[1.8] items-center justify-center overflow-hidden border-r border-[#333]">
        <div className="relative h-[520px] w-[420px]" style={{ perspective: `${camera.perspective}px` }}>
          <div
          ...
          >
            <div className="absolute inset-0 flex flex-col overflow-hidden rounded-2xl border border-[#3b3b3b] bg-[#1f1f1f] shadow-2xl">
              ...
              <div className="relative grid flex-1 place-items-center overflow-hidden">
                <div className="absolute inset-0 opacity-40" style={{ backgroundImage: 'radial-gradient(#333 1px, transparent 1px)', backgroundSize: '20px 20px' }} />
            ...
            >
              ...
              <div className="flex min-h-12 w-full items-center justify-center gap-3 rounded-lg border border-[#333] bg-[#151515] px-4 py-3">
                ...
                <span className="rounded bg-blue-600 px-2.5 py-0.5 font-mono text-sm font-bold text-white transition-transform duration-300" style={{ transform: `scale(${step >= 3 ? 1.1 : 1})` }}>
                  {likes}
                </span>
              </div>
            </div>
            ...
            >
              <div className="font-mono text-[13px] leading-relaxed text-gray-200">
                ...
                <div className="pl-6">
                  {'<'}<span className="text-blue-400">Card</span>{'>'}<br />
                  &nbsp;&nbsp;{'<'}<span className="text-blue-400">Status</span> /{'>'}<br />
                  &nbsp;&nbsp;{'<'}<span className="text-blue-400">Counter</span> /{'>'}<br />
            ...
            >
              <div className="text-center">
                <div className="mb-2 text-sm font-bold uppercase tracking-[0.2em] text-emerald-400">点赞数</div>
                <div className="inline-flex items-center gap-3 rounded-lg border border-emerald-400/40 bg-[#151515] px-4 py-2">
                  <Database size={16} className="text-emerald-400" />
                  <span className="font-mono text-2xl font-bold tabular-nums text-white">{likes}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <aside className="flex min-w-0 flex-[1.2] flex-col justify-between bg-[#1c1c1c] p-14">
        <div>
          <div className="mb-4 flex items-center gap-3">
            ...
            <div className="font-mono text-xl uppercase tracking-[0.2em] text-gray-500">Visualizer</div>
          </div>
          <div className="pl-10">
            <div className={`absolute mt-1 -ml-10 grid size-8 place-items-center rounded-full border bg-[#1c1c1c] ${activeTheme}`}>
              <ActiveIcon size={14} />
            </div>
            <div className="rounded-xl border border-[#333] bg-[#252525] p-8">
              <div className={`mb-3 font-mono text-lg font-bold uppercase tracking-wider ${activeTheme.split(' ')[0]}`}>
